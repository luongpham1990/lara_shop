                        ...
                        <table class="table table-striped table-bordered table-hover" id="dataTables-example">
                            ...
                            @foreach($product as $pro)
                                <tr class="product{{$pro->id}}">
                                    ...
                                    <td>{{$pro->view}}</td>
                                    <td>
                                        {{ \App\Catalog::find($pro->catalog_id)->first()->catalog_name }}
                                    </td>
                                    <td>
                                        <?php $img=DB::table('product_photos')
                                                ->select('product_photos.thumbnail_photo_link')
                                                ->join('product_product_photo','product_photos.product_photo_id', '=', 'product_product_photo.product_photo_id')
                                                ->join('products','product_product_photo.product_id','=','products.id')
                                                ->first();

                                        ?>
                                        <img class="img-responsive" style="max-width: 100px;" src="/images/{{ $img->thumbnail_photo_link }}" >
                                    </td>
